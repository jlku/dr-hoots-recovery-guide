...
2. John confirms or changes the provisional narrator voice for Spanish and Mandarin. To change it, set `voice` in `assets/audio/v2/voices.json`, run `node --env-file=.env.local scripts/generate-v2-narration.mjs --language es --force` (and `--language zh-Hans`), about $0.40 together, then `node scripts/build-segment-media.mjs`.
