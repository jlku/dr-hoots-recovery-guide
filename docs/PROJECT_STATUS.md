...
The same day, Spanish and Mandarin were narrated with George, and reviewed again by three-reviewer panels. The panels caught a Spanish sentence that made the patient the one touching the incision, an ambiguous "follow along" label, and a Mandarin lethargy threshold that read lower than the English. Every segment fits the 35-second narration ceiling in every language. The longest is Mandarin chapter 6, the phone numbers, at 33.9 s of speech and 36.3 s on screen. The simulated Song review passed this build, three of three, after four panels found and fixed a phone language menu squeezed to "Eng", a pending-medication banner that patients misread, seven chapters overflowing the rail in Spanish, and two gaps in its own evidence. `npm run export` now turns the six fixed chapters into eighteen verified videos, in three languages, in about four minutes.

Two findings on September 19 changed what the guide says and how it says it. The English recording had drifted from the reviewed text: in seven of eleven beats the audio said something other than the canonical sentences, and nothing caught it, because captions are generated from the recording. English was recorded again from the canonical text, and the narration validator now checks every language, English included. Separately, narration no longer speaks a value a patient's link can change: the follow-up sentence says the clinic will give the date, and the date itself is live text on the frame, with "about two weeks after surgery" shown when a link carries none. That sentence rests on a new kind of source, the practice dot phrase, which carries a file instead of a URL and stays in surgeon review until Song marks it up.
...
2. Song marks up the draft dot phrase on the provider page (`guide/provider.html`), confirms the presets, and supplies wording for patients who get no antibiotic or no prescription pain medicine.
