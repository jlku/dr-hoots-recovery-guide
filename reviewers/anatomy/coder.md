...
- unallowed_marks: each text, number, arrow, label, or symbol drawn on top of the scene that answer 3 reports and that is not one of the allowed marks, named in a few words of your own. A detail of something depicted, such as an indicator light on a device or a line on the skin, is not a mark, even when an observer mentions it in answer 3; an observer saying there is no text is not a mark either.
...
Give each distinct misreading one short kebab-case key, and reuse that key when different observers describe the same misreading in different words. For each key, describe the misreading in your own words, record the forbidden reading it amounts to, if any, and record harmful_action: whether a viewer who believed it might do something harmful, such as cutting, pressing on or pulling at the skin, tying something at or around the throat, or acting on a different part of the body than the picture shows. A misreading that only makes the picture look alarming, such as a line that looks like a wound, is not a harmful action, and neither is the reverse of the pictured action.
